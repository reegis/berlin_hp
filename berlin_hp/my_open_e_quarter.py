...
def get_alkis_with_additional_data():
    ...
    return pd.DataFrame(pd.read_hdf(filename_alkis, "alkis"))
...
def oeq(bzr=None):
    ...
    sn_data = pd.read_csv(
        os.path.join(cfg.get("paths", "data_berlin"), "data_by_blocktype.csv"),
        ";",
    )
    ...
    rename_alkis = {
        "AOG": "floors",
        "BEZGFK": "building_function_name",
        "GFK": "building_function",
        "SCHL5": "block",
        "PLR": "lor",
